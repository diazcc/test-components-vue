    ...
    <article class="template-table-view">
        <section class="template-table-view__content">
            <header class="template-table-view__header">
                <h1 class="template-table-view__header__title">{{t('historical_files')}}</h1>
                <Button v-if="hasPermissions.validator('create_file')" 
                    class="template-table-view__header__button" :dataButton="dataHistoricalFiles.dataButtonNewFiles" />
            </header>
            <section class="template-table-view__list">
                <nav class="template-table-view__list__nav">
                    <MultiSelect :dataMultiSelect="dataHistoricalFiles.dataMultiSelect" @onChange="applyFilters"/>
                    <InputSearch :dataInputSearch="dataHistoricalFiles.dataInputSearch" />
                </nav>
                <table class="table">
                    ...
                    <tbody class="table__row table--only-row loading-component" v-if="dataHistoricalFiles.stateLoadData">
                        <td class="table__row__cell">{{ t("loading") }}</td>
                    </tbody>
                    <tbody class="table__row table--only-row" v-else-if="dataHistoricalFiles.data.length < 1">
                        <td class="table__row__cell">{{ t("no_results_found") }}</td>
                    </tbody>
                    <tbody class="table__row" v-else v-for="(data, index) in dataHistoricalFiles.data" @click="dataHistoricalFiles.getFile(data)">
            ...
            <Paginator :dataPaginator="dataHistoricalFiles.dataPaginator"/>
        </section>
    </article>
    <ModalPdfViewer :dataModalPdfViewer="dataHistoricalFiles.dataModalPdfViewer" />
    <ModalAlert :dataModalAlert="dataHistoricalFiles.dataModalAlert" />
</template>
...
defineProps(['dataHistoricalFiles']);
...
<style scoped src="./HistoricalFiles.template.scss"></style>
